...
Compared to v0.3.0, three findings are no longer reported:
...
- **"Unused state variable" (`ENTRY_POINT_STORAGE_LOCATION`)**: resolved by the removal of the entire ERC-7201 storage system; the variable no longer exists.
- **"Empty `require()` statement"**: resolved by adding `EntryPointAddressZero()` to the constructor zero-address guard.
...
> Found: `onlyEntryPoint()` modifier (line 79).
