...
Three findings are unchanged from v0.3.0: H-1, L-3 (PUSH0), and L-4 (Modifier Invoked Only Once).

Two findings are new in v1.0.0:
- **L-1 "Unspecific Solidity Pragma"**: consequence of the CVF-6 fix (`^0.8.34`).
- **L-2 "Empty `require()` statement"**: consequence of the zero-address guard added to the constructor.
...
> Found: `onlyEntryPoint()` modifier (line 76).
